clc,clear
r=load('data109.txt');   %原始的相关系数矩阵保存在纯文本文件data109.txt中
n1=2;n2=3;num=min(n1,n2);
...
for i=1:n1
    ...
    vec1(:,i)=vec1(:,i)/sign(sum(vec1(:,i))); %特征向量乘±1,保证所有分量和为正
end
...
for i=1:n2
    ...
    vec2(:,i)=vec2(:,i)/sign(sum(vec2(:,i))); %特征向量乘±1，保证所有分量和为正
end
